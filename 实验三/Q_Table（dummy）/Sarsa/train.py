...
def train(cfg, env, agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while True:
            ...
            next_action = agent.choose_action(next_state)  # 根据算法选择一个动作
            if i_ep % cfg.render_frqc == 0 and i_ep != 0:
                env.render()  # 渲染动作并显示
            agent.update(state, action, reward, next_state, next_action, done)  # Q-learning算法更新
...
if __name__ == "__main__":
    ...
    # 测试智能体
    # 因为是测试（应用），将epsilon设置成0，省掉“探索”的过程，直接“利用”
    agent.epsilon = 0
